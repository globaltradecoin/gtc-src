...
#else
    const char* pszModule = "GlobalTradeCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GlobalTradeCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GlobalTradeCoin
    // Mac: ~/Library/Application Support/GlobalTradeCoin
    // Unix: ~/.GlobalTradeCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GlobalTradeCoin";
#else
    fs::path pathRet;
    ...
    return pathRet / "GlobalTradeCoin";
#else
    // Unix
    return pathRet / ".GlobalTradeCoin";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "GlobalTradeCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "GlobalTradeCoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong GlobalTradeCoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("GlobalTradeCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
