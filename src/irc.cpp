...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("GlobalTradeCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #GlobalTradeCoinTEST2\r");
            Send(hSocket, "WHO #GlobalTradeCoinTEST2\r");
        } else {
            // randomly join #GlobalTradeCoin00-#GlobalTradeCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #GlobalTradeCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #GlobalTradeCoin%02d\r", channel_number).c_str());
        }
